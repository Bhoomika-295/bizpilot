...
import { ArrowLeft, Ban, CalendarClock, CheckCircle2, CircleDashed, Clock3, ListChecks, Loader2, LockKeyhole, Pencil, Play, Plus, RefreshCw, RotateCcw, ShieldCheck, Target, TimerReset, XCircle, Activity, GitBranch } from "lucide-react";
...
export default function ActionsPage() {
  ...
  const [reviewActualResult, setReviewActualResult] = useState("");
  const [varianceStatus, setVarianceStatus] = useState("UNKNOWN");
  const [decisionEffectiveness, setDecisionEffectiveness] = useState("INCONCLUSIVE");
  ...
  const executionReviewMutation = trpc.actionPlans.executionReview.useMutation({ onSuccess: () => { toast.success("Execution health reviewed"); refresh(); }, onError: (error) => toast.error(error.message) });
  const reviewOutcomeMutation = trpc.actionPlans.reviewOutcome.useMutation({ onSuccess: () => { toast.success("Outcome reviewed and learning refreshed"); setIsReviewOpen(false); setReviewActualResult(""); refresh(); }, onError: (error) => toast.error(error.message) });
  ...
  const latestOutcome: any = detailQuery.data?.outcomes?.[0];
  ...
  const pending = createMutation.isPending || editMutation.isPending || approveMutation.isPending || startMutation.isPending || blockMutation.isPending || unblockMutation.isPending || completeMutation.isPending || cancelMutation.isPending || reopenMutation.isPending || executionReviewMutation.isPending || reviewOutcomeMutation.isPending;
  ...
  return (
    <DashboardLayout>
      ...
      <Dialog open={isReviewOpen} onOpenChange={setIsReviewOpen}><DialogContent className="sm:max-w-lg"><DialogHeader><DialogTitle>Review outcome and learning</DialogTitle><DialogDescription>Compare the observed result with the expected result and confirm the review without inferring unsupported causation.</DialogDescription></DialogHeader><div className="space-y-4"><div><Label htmlFor="review-result">Actual result summary</Label><Textarea id="review-result" value={reviewActualResult} onChange={(event) => setReviewActualResult(event.target.value)} placeholder="What was observed, with evidence or caveats?" rows={4} /></div><div className="grid grid-cols-1 gap-3 sm:grid-cols-3"><div><Label>Variance</Label><Select value={varianceStatus} onValueChange={setVarianceStatus}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="MATCHED">Matched</SelectItem><SelectItem value="BETTER">Better</SelectItem><SelectItem value="WORSE">Worse</SelectItem><SelectItem value="MIXED">Mixed</SelectItem><SelectItem value="UNKNOWN">Unknown</SelectItem></SelectContent></Select></div><div><Label>Decision effectiveness</Label><Select value={decisionEffectiveness} onValueChange={setDecisionEffectiveness}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="EFFECTIVE">Effective</SelectItem><SelectItem value="PARTIALLY_EFFECTIVE">Partially effective</SelectItem><SelectItem value="INEFFECTIVE">Ineffective</SelectItem><SelectItem value="INCONCLUSIVE">Inconclusive</SelectItem></SelectContent></Select></div><div><Label>Confidence</Label><Select value={reviewConfidence} onValueChange={setReviewConfidence}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="HIGH">High</SelectItem><SelectItem value="MEDIUM">Medium</SelectItem><SelectItem value="LOW">Low</SelectItem></SelectContent></Select></div></div></div><DialogFooter><Button variant="outline" onClick={() => setIsReviewOpen(false)}>Cancel</Button><Button disabled={reviewOutcomeMutation.isPending || reviewActualResult.trim().length < 3 || !selectedAction || !latestOutcome} onClick={() => { const outcome = latestOutcome; if (selectedAction && outcome) reviewOutcomeMutation.mutate({ businessId, actionPlanId: selectedAction.id, outcomeId: outcome.id, actualResultSummary: reviewActualResult, varianceStatus: varianceStatus as "MATCHED" | "BETTER" | "WORSE" | "MIXED" | "UNKNOWN", decisionEffectiveness: decisionEffectiveness as "EFFECTIVE" | "PARTIALLY_EFFECTIVE" | "INEFFECTIVE" | "INCONCLUSIVE", reviewConfidence: reviewConfidence as "HIGH" | "MEDIUM" | "LOW", assumptionsReview: [], dependencyReview: [] }); }} className="bg-slate-900 text-white">{reviewOutcomeMutation.isPending && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}Confirm review</Button></DialogFooter></DialogContent></Dialog>
    </DashboardLayout>
  );
}
