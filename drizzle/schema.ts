...
export const outcomes = mysqlTable(
  "outcomes",
  {
    ...
    actualResultSummary: text("actualResultSummary"),
    varianceStatus: varchar("varianceStatus", { length: 30 }).notNull().default("UNKNOWN"), // MATCHED, BETTER, WORSE, MIXED, UNKNOWN
    assumptionsReviewJson: text("assumptionsReviewJson"),
    dependencyReviewJson: text("dependencyReviewJson"),
    decisionEffectiveness: varchar("decisionEffectiveness", { length: 30 }), // EFFECTIVE, PARTIALLY_EFFECTIVE, INEFFECTIVE, INCONCLUSIVE
    reviewConfidence: varchar("reviewConfidence", { length: 30 }), // HIGH, MEDIUM, LOW
    reviewedByUserId: int("reviewedByUserId"),
    reviewedAt: timestamp("reviewedAt"),
    lessonMemoryId: int("lessonMemoryId"),
    createdAt: timestamp("createdAt").defaultNow().notNull(),
    updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
  },
  (table) => ({
...
export const decisionCandidates = mysqlTable(
  "decisionCandidates",
  {
    ...
    tradeOffsJson: text("tradeOffsJson"),
    qualityMetricsJson: text("qualityMetricsJson"),
    makerUserId: int("makerUserId"),
    makerNote: text("makerNote"),
    versionNumber: int("versionNumber").notNull().default(1),
    parentDecisionId: int("parentDecisionId"),
...
export const actionPlans = mysqlTable(
  "actionPlans",
  {
    ...
    decisionId: int("decisionId"),
    strategyId: int("strategyId"),
    dependencyIdsJson: text("dependencyIdsJson"),
    expectedResult: text("expectedResult"),
    executionHealth: varchar("executionHealth", { length: 30 }).notNull().default("UNKNOWN"), // HEALTHY, AT_RISK, BLOCKED, OVERDUE, UNKNOWN
    executionHealthReason: text("executionHealthReason"),
    blockedDurationHours: int("blockedDurationHours").notNull().default(0),
    lastExecutionReviewAt: timestamp("lastExecutionReviewAt"),
